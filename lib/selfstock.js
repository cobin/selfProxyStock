...
var SelfProxyStock = function(settings,logger){
		...
		this.sync = function(req,res){
			...
			request({ uri:sUrl+"?msisdn="+msisdn+param, timeout:30000 }, function (error, response, body) {
				if(error || response.statusCode != 200) {
					console.log(["request-error-1",sUrl,error?error:response.statusCode]);
					res.send('{"error":"获取自选股失败","result":null}');
				} else {
					if(body && body.length>5){
						msisdn = req.query.msisdn;
						var stocks = body.replace(/,{1,}$/g,'').replace(/^,{1,}/g,'');
						param = "&type=2&stocklist="+stocks;
						console.log(["INFO-FAKE",req.params.fakecode,msisdn,stocks]);
						request({ uri:sUrl+"?msisdn="+msisdn+param, timeout:30000 }, function (error, response, body) {
							if(error || response.statusCode != 200) {

...
var SelfProxyStock = function(settings,logger){
	  ...
	  this.logger = logger;
		...
		var requestRemoteSelfStock=function(param,req,res){
			request({ uri:selfstock.fakecodeurl+req.fakecode, timeout:10000 }, function (error, response, body) {
				 if(error || response.statusCode != 200) {
				 		console.log(["request-error",selfstock.fakecodeurl,req.fakecode,error]); 
				 		res.send("");
				 }else{
						var msisdn = body;
						if(body==null || body.length<11){
							...
							request({ uri:sUrl, timeout:30000 }, function (error, response, body) {
						      if(error || response.statusCode != 200) {
						          ...
						          try {
						          	logger.log(["info",msisdn,param,body]);
						            res.send(body);
						          } catch(err) {
...
exports.createSelfProxyStock =function(settings,logger){
	return new SelfStock(settings,logger);
}
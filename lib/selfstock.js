...
var SelfProxyStock = function(settings,logger){
		...
		this.add = function(req,res) {
			var stocklist = req.query.stocks;
			if(stocklist && stocklist.length>2){
				stocklist = stocklist.replace(/[,]{2,}/g,',').replace(/^,|,$/g,'');
				var param = "&type=11&stocklist="+stocklist;
				requestRemoteSelfStock(param,req,res,2);
			}else{
				res.send('{"error":"缺少股票代码","result":null}');
			}
		};
		...
		this.deletes = function(req,res) {
			...
			if(stocklist && stocklist.length>2){
				stocklist = stocklist.replace(/[,]{2,}/g,',').replace(/^,|,$/g,'');
				var param = "&type=12&stocklist="+stocklist;
				requestRemoteSelfStock(param,req,res,1);
			}else{
				res.send('{"error":"缺少股票代码","result":null}');
			}

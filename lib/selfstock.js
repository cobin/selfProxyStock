...
var SelfProxyStock = function(settings,logger){
	  ...
	  this.logger = logger;
		...
		var requestRemoteSelfStock=function(param,req,res){
			request({ uri:selfstock.fakecodeurl+req.params.fakecode, timeout:10000 }, function (error, response, body) {
				 if(error || response.statusCode != 200) {
				 		console.log(["request-error",selfstock.fakecodeurl,req.params.fakecode,error]); 
				 		res.send("");
				 }else{
						var msisdn = body;

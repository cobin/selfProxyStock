...
var SelfProxyStock = function(settings,logger){
	  ...
	  this.logger = logger;
		...
		var requestRemoteSelfStock=function(param,req,res){
			console.log(["info",req.params.fakecode]);
			request({ uri:_self.selfstock.fakecodeurl+req.params.fakecode, timeout:10000 }, function (error, response, body) {
				 ...
				 if(error || !response.statusCode || response.statusCode != 200) {
				 		console.log(["request-error",_self.selfstock.fakecodeurl,req.params.fakecode,error,response.statusCode?response.statusCode:null]); 
				 		res.send("");
				 }else{
						var msisdn = body;
						...
						if(body==null || body.length<11){
							...
							console.log(["info",sUrl]);
							request({ uri:sUrl, timeout:30000 }, function (error, response, body) {
								  console.log(error);
						      if(error || !response.statusCode ||  response.statusCode != 200) {
						          console.log(["request-error",sUrl,error,response.statusCode?response.statusCode:null]); 
						          res.send("");
						      } else {
						          try {

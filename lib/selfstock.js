						...
						request({ uri:sUrl+"?msisdn="+msisdn+param, timeout:30000 }, function (error, response, body) {//请求获取手机号码对应的股票列表
							if(error || response.statusCode != 200) {
								...
								for(var j=0;j<mstocks.length;j++){
									fstocks = fstocks.replace(","+mstocks[j]+",",",");
								}
								fstocks = fstocks.replace(/,{1,}$/g,'').replace(/^,{1,}/g,'');
								param = "&type=2&stocklist="+fstocks;
								console.log(["INFO-FAKE-Add",req.params.fakecode,msisdn,fstocks]);
